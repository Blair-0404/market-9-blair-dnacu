...
function isValidatedId() {
  ...
  if (
    idValue.length < VALIDATION_LENGTHS.ID_MIN_VALUE ||
    idValue.length > VALIDATION_LENGTHS.ID_MAX_VALUE
  ) {
...
function checkControlOtherList() {
  signUpSelectors.checkAllTerm.checked =
    signUpSelectors.checkNecessaryList.checked &&
    signUpSelectors.checkAdAgree.checked;
}
